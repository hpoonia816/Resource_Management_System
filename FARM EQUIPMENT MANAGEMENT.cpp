...
public:
    ...
    void print()
    {
        ...
        cout << "\nFarmer\t Allocation\t Need\t\tMax\t\t Available\t";
    ...
    void add_farmer(Farmer &f1)
    {
        ...
        max_matrix.push_back(f1.request);
        vector<int> need;
...
for(int i = 0 ; i < number_of_resources ; i++)
    ...
    void safe_sequence() const
    {
        ...
        vector<int> work(r);
        vector<int> temp_avail = available_resources;
        for (int i = 0; i < r ; i++)
            work[i] = temp_avail[i];
        int count = 0;
        ...
        for (int i = 0; i < n-1 ; i++)
            cout << "Farmer" << safeSequence[i] + 1 << " -> ";
        cout << "Farmer" << safeSequence[n - 1] + 1 << "\n";
    }
    void remove_deadlock()
    {
        ...
        for(int j = 0 ; j < no_of_farmers ; j++)
        {
            for(int k = 0 ; k < number_of_resources ; k++)
            {
                if(max_matrix[j][k] > max_possible[k])
                {
            ...
            for(int j = 0 ; j < no_of_farmers ; j++)
            {
                if(i & (1<<j))
                {
                    for(int k = 0 ; k < number_of_resources ; k++)
                    {
                        ...
                        needed_resources[j][k] = max_matrix[j][k];
                    }
                }
            }
            if(!deadlock_check())
        ...
        cout<<"After De-allocation of resources from the farmers mentioned above , data is :\n";
        print();
        safe_sequence();
        available_resources = avail_rs;
        allocated_resources = temp;
...
int main()
{
    ...
    vector<int> availResources(noOfResources);
    for(int i=0; i < noOfResources; i+=1){
        cin >> availResources[i];
    }
    manager man(noOfResources , availResources);
    int x = 1;
    ...
    vector<int> currently_allocated(noOfResources);
    vector<int> maximum_required(noOfResources);
    while( x< 4)
    {
        if(x == 1)
        {
            cout<<"Enter resources currently allocated to farmer \n";
            for(int i = 0 ; i < noOfResources ; i++)
            {
                cin >> currently_allocated[i];
            }
            cout<<"Enter maximum resources demanded by farmer \n";
            for(int i = 0 ; i < noOfResources ; i++)
            {
                cin >> maximum_required[i];
            }
            Farmer f(currently_allocated , maximum_required);
            farmers.push_back(f);
            man.add_farmer(f);
            cout<<"Press 1 for adding farmer\nPress 2 for checking safe state and getting safe sequence\nPress 3 to print available date\nPress 4 to exit. \n";
            cin>>x;
        ...
        else if(x == 2)
        {
            ...
            else
            {
                ...
                int y;
                cout<<"Press 1 to remove deadlock and print safe sequence, any other key to continue\n";
                cin>>y;
                if(y == 1)
                {
                    man.remove_deadlock();
